...
fn decode_vp8_ivf(data: &[u8]) -> Result<Vec<videoson_core::VideoFrame>> {
    ...
    let mut dec = Vp8Decoder::try_new(&track.codec_params, &opts)?;
...
#[test]
fn decode_vp8_from_ivf() -> Result<()> {
    let path = concat!(env!("CARGO_MANIFEST_DIR"), "/tests/samples/test_vp8.ivf");
    let data = std::fs::read(path)?;
    let frames = decode_vp8_ivf(&data)?;

    assert!(!frames.is_empty(), "expected at least 1 VP8 frame");
...
#[test]
fn decode_vp8_nv12_output() -> Result<()> {
    let path = concat!(env!("CARGO_MANIFEST_DIR"), "/tests/samples/test_vp8.ivf");
    let data = std::fs::read(path)?;
